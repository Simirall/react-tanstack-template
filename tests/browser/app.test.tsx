...
import { cleanupRenderedApp, renderApp } from "./test-utils";
...
test("localStorage に保存された認証状態からホーム画面を復元できる", async () => {
  renderApp({ route: "/login" });
  ...
  await expect.poll(() => window.location.pathname).toBe("/");
  expect(localStorage.getItem("auth")).toBe('{"isLogin":true}');

  cleanupRenderedApp({ preserveStorage: true });
  renderApp({ preserveStorage: true, route: "/" });
